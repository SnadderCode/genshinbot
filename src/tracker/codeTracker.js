...
async function collectCodesFromSources() {
    ...
    for (const source of sources) {
        try {
            ...
            console.error(
                `[CODE SOURCE] Failed to fetch codes from ${source.name}:`,
                error
            );
        }
    }
...
async function loadSavedCodes() {
    try {
        ...
        console.error(
            "[CODE TRACKER] Failed to load saved codes:",
            error
        );
...
async function findNewCodes() {
    try {
        ...
        console.error(
            "[CODE TRACKER] Failed to find new codes:",
            error
        );
...
async function saveNewCodes(codes) {
    try {
        ...
        console.error(
            "[CODE TRACKER] Failed to save codes:",
            error
        );
